...
STAGE1_TOPN   = 20    # 量价筛选后进入第二阶段的股票数
...
BANNER = """
╔══════════════════════════════════════════════════════╗
║  capital_flow_screener  v4  |  Tushare 稳定版        ║
...
def screen_stage1(df: pd.DataFrame, hs300_chg: float,
    ...
    """
    if hs300_chg <= -2.0:
        mkt_mode, pct_lo, pct_hi, vol_thresh, score_pass = "弱市",  0.5, 9.0, 1.5, 3
    elif hs300_chg <= 0:
        mkt_mode, pct_lo, pct_hi, vol_thresh, score_pass = "震荡",  1.5, 9.0, 1.8, 4
    else:
        mkt_mode, pct_lo, pct_hi, vol_thresh, score_pass = "强市",  2.5, 9.0, 2.0, 5

    print(f"  [评分] 大盘【{mkt_mode}】→ 涨幅预筛 {pct_lo}%~{pct_hi}%，量比≥{vol_thresh}")
...
def _score_one(row: dict, hs300_chg: float, hist=None) -> tuple[int, list]:
    ...
    if hist is not None and len(hist) >= 6:
        ...
        if len(close) >= 60:
            ...
            if pd.notna(ma20) and pd.notna(ma60) and close.iloc[-1] > ma20 > ma60:
                hits.append("⑥价格>MA20>MA60")
        elif len(close) >= 20:
            ma20 = close.rolling(20).mean().iloc[-1]
...
def save_excel(combined: list[dict], out_path: pathlib.Path):
