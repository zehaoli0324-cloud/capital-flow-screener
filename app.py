...
修复与新增：
  ...
  5. 【稳定性增强】工作线程异常捕获 + Tushare 连接超时，防止任务永久卡死。
"""
...
class _StdoutRouter:
    ...
    def _read_loop(self):
        ...
        try:
            ...
            # 读取线程异常，静默记录（不影响主流程）
            _tlog(self._task_id, f"[内部警告] 日志捕获线程异常: {e}")
        # 处理剩余
        if buf:
...
if not _thread_started:
    ...
    print("[INFO] Heavy worker thread started.", file=sys.stderr)
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                ...
                try:
                    ...
                    log("❌ Tushare 连接超时（10秒），请检查网络或 Token")
                    with _tasks_lock:
                        _tasks[task_id]["status"] = "error"
                    return
